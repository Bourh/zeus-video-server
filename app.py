...
def make_video(sd,vid):
 ...
 text='\n'.join(textwrap.wrap(sd.get('script','فلسفة ديزاد'),width=28)[:8]).replace("'",' ').replace(':',' ').replace('%',' ')
 cmd=['ffmpeg','-y','-f','lavfi','-i','color=c=#0f0f23:size=1080x1920:duration=40:rate=30','-vf',f"drawtext=text='{emoji}':fontsize=160:x=(w-text_w)/2:y=180:fontcolor=white,drawtext=text='{char}':fontsize=52:x=(w-text_w)/2:y=400:fontcolor=#FFD700,drawtext=text='{text}':fontsize=40:x=70:y=620:fontcolor=white:line_spacing=12,drawtext=text='فلسفة ديزاد':fontsize=36:x=(w-text_w)/2:y=1800:fontcolor=#555555",'-c:v','libx264','-preset','ultrafast','-crf','28','-t','40',out]
...
@app.route('/')
def home():return jsonify({'status':'✅ فلسفة ديزاد','youtube':'✅' if os.path.exists(TOKENS_FILE) else'❌ /auth'})
@app.route('/auth')
def auth():
